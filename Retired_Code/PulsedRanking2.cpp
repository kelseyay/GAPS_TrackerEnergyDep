    ...
    for(int r = 0; r < nrows;r++){
        for(int m = 0; m < nmods;m++){
            for(int s = 0; s < nstrips; s++){
                if(hpulser->GetBinContent(r*32+s+1,l*6+m+1)> 2.5){cout << "L" << l << "R" << r << "M" << m << "S" << s << " Pulsed!" << endl; }
                //if(hpulser->GetBinContent(r*32+s+1,l*6+m+1)> 2.5) myfile << l << r << m << "\t" << s << endl;
            }
        }
    }
}
...
for(int x = 0; x < nrows*nstrips;x++){
    for(int y = 0; y < nlayers*nmods;y++){
        ...
        if(hsig_nocorr->GetBinContent(x+1,y+1)!= 0){hdiff->Fill(row*ncn+ncn_val,layer*nmods+mod,0.1);} //{hdiff->Fill(row*ncn+ncn_val,layer*nmods+mod);}
    }
}
