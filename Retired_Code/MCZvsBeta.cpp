//To use: ./MCZvB -i /home/kelsey/simulations/simdat/mu/v.3.0.0/triggerlevel1/mu-_gaps_triggerlevel1_FTFP_BERT_1757850432_rec -o test2 -l 0.6 -u 1 -b 20 -p 13
...
float rtkr = 2.33;
float Gtkrnew = 1; //1; // (1/0.83);

//Prepare textile for saving values
std::ofstream myfile;
myfile.open(out_path + "Z_vs_Beta.txt");
...
for(int i = 0; i < bbins; i++){
    ...
    if(i != bbins - 1) myfile << ",";
    htkr[i] = new TH1F (("htkr"+to_string(i)).c_str(), ("Z Calc Beta " + to_string(betacut+bwid*i) + " - " + to_string(betacut+bwid*(i+1)) ).c_str(), NBins, xlow,xhigh);
    htof[i] = new TH1F (("htof"+to_string(i)).c_str(), ("Z Calc Beta " + to_string(betacut+bwid*i) + " - " + to_string(betacut+bwid*(i+1)) ).c_str(), NBins, xlow,xhigh);
}
myfile << endl;
...
for(int i = 0; i < cbins; i++){
    ...
    if(i != cbins - 1) myfile << ",";
    htof_cos[i] = new TH1F (("htof_cos"+to_string(i)).c_str(), ("MIP Z Calc Cos " + to_string(coslow+cwid*i) + " - " + to_string(coslow+cwid*(i+1)) ).c_str(), NBins, xlow,xhigh);
    htkr_cos[i] = new TH1F (("htkr_cos"+to_string(i)).c_str(), ("MIP Z Calc Cos " + to_string(coslow+cwid*i) + " - " + to_string(coslow+cwid*(i+1)) ).c_str(), NBins, xlow,xhigh);
}
myfile << endl;

//TH2F(name, title, nbinsx, xlow, xup, nbinsy, ylow, yup)
auto h2dbetaTKR = new TH2F("h2dbetaTKR","Calculated Z Distribution vs Beta" ,bbins,betacut,betahigh,NBins, xlow,5);
auto h2dbetaTOF = new TH2F("h2dbetaTOF","Calculated Z Distribution vs Beta" ,bbins,betacut,betahigh,NBins, xlow,5);
auto h2dcosTOF = new TH2F("h2dcosTOF","Calculated Z vs Cos(Theta)" ,cbins,coslow,coshigh,NBins, xlow,5);
auto h2dcosTKR = new TH2F("h2dcosTKR","Calculated Z vs Cos(Theta)" ,cbins,coslow,coshigh,NBins, xlow,5);
...
for(unsigned int i = 0; i < TreeRec->GetEntries(); i+=MainLoopScaleFactor){
	...
	if(pt != nullptr && -fabs(MCEvent->GetPrimaryMomentumDirection().CosTheta()) < -fabs(coslow) && -fabs(MCEvent->GetPrimaryMomentumDirection().CosTheta()) > -fabs(coshigh) && MCEvent->GetPrimaryBeta()*MCEvent->GetPrimaryMomentumDirection()[2] < 0 && fabs(MCEvent->GetPrimaryBeta()) >  betacut && fabs(MCEvent->GetPrimaryBeta()) <  betahigh ){
        ...
        //Now you have three vectors indexed by k: MCVolid - volume id of hit, MCEdep - total energy deposition into the volume id,  MCSpec - species of each index, prioritizing the primary species first
        ...
        if(MCST){ //Monte Carlo single track requirement
            int TrueUMBflag = 0;
            int TrueCORflag = 0;
            ...
            //Take another look at this; it might be what's giving you trouble!
            int TOF_ct = 0; //Require NHits > 3 to calculate Z
            int TKR_ct = 0; //Require NHits > 3 to calculate Z

            vector<double> Zevent_TOF;
            vector<double> Zevent_TKR;
            ...
            for (int k = 0; k < MCVolid.size(); k++) {
                if(MCEdep[k] > 0.4){
                    ...
                    if(GGeometryObject::IsTofVolume(MCVolid[k])) TOF_ct++;
                    if(GGeometryObject::IsTrackerVolume(MCVolid[k])) TKR_ct++;
            ...
            if(TrueUMBflag > 0 && TrueUMBflag < 3 && TrueCBEtopflag > 0 && TrueCBEtopflag < 3 ){ //Require one or two hits in the TOF section
                ...
                for (int k = 0; k < MCVolid.size(); k++) { //Iterating over the hit series of a single track event (MC)
                    if(MCEdep[k] > 0.4){
                        ...
                        if(GGeometryObject::IsTofVolume(VolumeId) && MCEdep[k] > TofCut && TOF_ct >= 3){
                            double ion = 0.000016*pow(Ztof,0.9);
                            double eps_noz = 0.15355*(Ztof/Atof)*(rtof*Ltof)*(1/pow(beta,2));
                            double const_terms = 0.2 - pow(beta,2) + log( 1.022*pow(beta,2)/( ion*(1 - pow(beta,2))) )  ;

                            if(volspec(VolumeId,2,1) == 0 || volspec(VolumeId,2,1) == 1){
                                double edep = MCEdep[k]*fabs(costheta);
                                Zevent_TOF.push_back( sqrt( (ion/eps_noz)*exp( boost::math::lambert_w0( (edep/ion)*exp(const_terms) ) - const_terms  ) ) );
                                if(print) cout << "Flat paddle hit!! " << VolumeId << endl;
                            }else{
                                double edep = MCEdep[k]*sqrt(1-pow(costheta,2));
                                Zevent_TOF.push_back( sqrt( (ion/eps_noz)*exp( boost::math::lambert_w0( (edep/ion)*exp(const_terms) ) - const_terms  ) ) );
                                if(print) cout << "Vertical paddle hit!! " << endl;
                            }
                        }

                        if(GGeometryObject::IsTrackerVolume(VolumeId) && MCEdep[k] > TrackerCut && TKR_ct >= 3){
                            double ion = 0.000016*pow(Ztkr,0.9);
                            double eps_noz = 0.15355*(Ztkr/Atkr)*(rtkr*Ltkr)*(1/pow(beta,2));
                            double const_terms = 0.2 - pow(beta,2) + log( 1.022*pow(beta,2)/( ion*(1 - pow(beta,2))) )  ;
                            double edep = MCEdep[k]*fabs(costheta);
                            Zevent_TKR.push_back( sqrt( (ion/eps_noz)*exp( boost::math::lambert_w0( (edep/ion)*exp(const_terms) ) - const_terms  ) ) );
                        }

                    }//Closed bracket for 0.4 MeV requirement
                }//Closed bracket requirement for iteration over hit series

                //Fill Z histograms here and do truncate mean calculaton:
 			    std::sort(Zevent_TOF.begin(), Zevent_TOF.begin()+Zevent_TOF.size());
				double TrZ_TOF = 0;
				double CtrTrZ_TOF = 0;

				if(Zevent_TOF.size() < 3){
				    TrZ_TOF = 0;
				}else{
			        for(unsigned int isig = 0; isig < double(Zevent_TOF.size())/2; isig++){
						TrZ_TOF += Zevent_TOF.at(isig);
						CtrTrZ_TOF++;
					}
					if(CtrTrZ_TOF == 0){ TrZ_TOF = 0;
					}else{ TrZ_TOF /= CtrTrZ_TOF;}
				}

				htof[bbin]->Fill(TrZ_TOF);
				h2dbetaTOF->Fill(beta,TrZ_TOF);
				if(beta > 0.8 && beta < 1.0){ //If beta is in MIP range
				    htof_cos[cbin]->Fill( TrZ_TOF );
                    h2dcosTOF->Fill( costheta, TrZ_TOF );
				}
                ...
                if(Zevent_TKR.size() < 3){
                    TrZ_TKR = 0;
                }else{
                    for(unsigned int isig = 0; isig < double(Zevent_TKR.size())/2; isig++){
                        TrZ_TKR += Zevent_TKR.at(isig);
                        CtrTrZ_TKR++;
                    }
                    if(CtrTrZ_TKR == 0){ TrZ_TKR = 0;
                    }else{ TrZ_TKR /= CtrTrZ_TKR;}
                }

                htkr[bbin]->Fill(TrZ_TKR);
                h2dbetaTKR->Fill(beta,TrZ_TKR);
                if(beta > 0.8 && beta < 1.0){
                    htkr_cos[cbin]->Fill( TrZ_TKR );
                    h2dcosTKR->Fill( costheta, TrZ_TKR );
                }
...
histplot2f_pts_errs("c0", h2dbetaTOF,pts_TOF,errs_TOF,"MC True: TOF Energy x Sin/Cos(theta) Distribution vs Beta", "Beta" , "Energy x Cos(theta)", "NEntries", out_path + "TOF2D_Z_vs_Beta" );
histplot2f_pts_errs("c1", h2dbetaTKR,pts,errs, "MC True: TKR Energy x Cos(theta) Distribution vs Beta", "Beta" , "Energy x Cos(theta)", "NEntries", out_path + "Tracker2D_Z_vs_Beta" );
histplot2f_pts_errs("c2", h2dcosTOF,cos_pts_TOF,cos_errs_TOF,"MC True: MIP TOF Energy x Sin/Cos(theta) Distribution vs Cos(Theta)", "Cos(Theta)" , "Angle Corrected Energy", "NEntries", out_path + "TOF2D_Z_vs_Cos" );
histplot2f_pts_errs("c3", h2dcosTKR,cos_pts_TKR,cos_errs_TKR,"MC True: MIP TKR Energy x Sin/Cos(theta) Distribution vs Cos(Theta)", "Cos(Theta)" , "Energy x Cos(Theta)", "NEntries", out_path + "TKR2D_Z_vs_Cos" );
...
for(int i = 0; i < bbins; i++){
    ...
    if(i != bbins - 1) myfile << ",";
    histplot1f(("ctof" + to_string(i)).c_str(), htof[i], ( "Calculated Z, Beta " + to_string(betacut+bwid*i) + " - " + to_string(betacut+bwid*(i+1)) ).c_str(),"Calculated Z TOF","NEvents", out_path + ("FulltofZ" + to_string(i)).c_str() );
}
...
for(int i = 0; i < bbins; i++){
    ...
    if(i != bbins - 1) myfile << ",";
    histplot1f(("ctkr" + to_string(i)).c_str(), htkr[i], ( "Calculated Z, Beta " + to_string(betacut+bwid*i) + " - " + to_string(betacut+bwid*(i+1)) ).c_str(),"Calculated Z TKR","NEvents", out_path + ("FulltkrZ" + to_string(i)).c_str() );
}

myfile << endl;

for(int i = 0; i < cbins; i++){
    histplot1f(("ctof_cos" + to_string(i)).c_str(), htof_cos[i], ( "MIP Z Cos " + to_string(coslow+cwid*i) + " - " + to_string(coslow+cwid*(i+1)) ).c_str(),"Z vs Cos(theta)","NEvents", out_path + ("FulltofZ_Cos" + to_string(i)).c_str() );
    histplot1f(("ctkr_cos" + to_string(i)).c_str(), htkr_cos[i], ( "MIP Z Cos " + to_string(coslow+cwid*i) + " - " + to_string(coslow+cwid*(i+1)) ).c_str(),"Z vs Cos(theta)","NEvents", out_path + ("FulltkrZ_Cos" + to_string(i)).c_str() );
}
