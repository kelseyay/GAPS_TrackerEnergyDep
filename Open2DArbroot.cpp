...
parser->AddCommandLineOption<int>("zmin", "Minimum Z value 2D Histogram", 1, "m");
parser->AddCommandLineOption<int>("zmax", "Maximum Z value 2D Histogram", 100, "x");
...
h = (TH2F*)f->Get("hnentries;1");
h->SetMaximum(Zmax);
h->SetMinimum(Zmin);

histplot2f("c1",h,"Full Tracker Strip Level NHits","row(0-5)*32 + strp (0-31)","layer(0-6)*6 + mod(0-5)","NEntries", "FullTkrNhits" );
